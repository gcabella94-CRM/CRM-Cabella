...
export function bindAgendaNavAndFilters(ctx) {
  ...
  document.getElementById('agenda-new-appointment')?.addEventListener('click', () => {
    if (typeof ctx?.creaNuovoAppuntamentoDaBottone === 'function') ctx.creaNuovoAppuntamentoDaBottone();
  });
...
// ===== Facade Init =====
let __agendaInited = false;

function bindAgendaMonthJump() {
  const cont = document.getElementById('agenda-month-summary');
  if (!cont) return;
  if (cont.dataset.monthJumpBound === '1') return;
  cont.dataset.monthJumpBound = '1';

  cont.addEventListener('click', (e) => {
    const cell = e.target?.closest?.('.agenda-month-day');
    if (!cell) return;
    const iso = cell.dataset?.date;
    if (!iso) return;
    // Preferisci il bridge legacy se esiste (compat con versioni approvate)
    if (window.AgendaLegacy && typeof window.AgendaLegacy.setWeekAnchor === 'function') {
      window.AgendaLegacy.setWeekAnchor(iso);
      window.AgendaLegacy.renderWeek?.();
      window.AgendaLegacy.scrollToWeek?.();
      return;
    }
    // fallback: usa l'anchor del modulo e renderizza (se il chiamante passa ctx)
    try {
      setAgendaWeekAnchor(new Date(iso));
    } catch {}
  });
}

export function initAgenda(ctx = {}) {
  if (__agendaInited) return;
  __agendaInited = true;
  try { bindAgendaNavAndFilters(ctx); } catch {}
  try { bindAgendaMonthJump(); } catch {}
}
