...
// Evita doppi addEventListener quando il legacy chiama il bind più volte.
let __agendaNavBound = false;

function bindOnce(el, type, handler, key) {
  if (!el) return;
  const k = key || `__bound_${type}`;
...
export function bindAgendaNavAndFilters(ctx) {
  // Il legacy può chiamare questa funzione più volte: bind una sola volta.
  if (__agendaNavBound) return;
  __agendaNavBound = true;

  // navigazione settimana + filtri
  bindOnce(document.getElementById('agenda-prev-week'), 'click', () => {
    agendaWeekAnchor = addDays(agendaWeekAnchor, -7);
    renderAgendaWeek(ctx);
  }, '__agenda_prev');

  bindOnce(document.getElementById('agenda-next-week'), 'click', () => {
    agendaWeekAnchor = addDays(agendaWeekAnchor, 7);
    renderAgendaWeek(ctx);
  }, '__agenda_next');

  bindOnce(document.getElementById('agenda-today-week'), 'click', () => {
    agendaWeekAnchor = startOfWeek(new Date());
    renderAgendaWeek(ctx);
  }, '__agenda_today');

  bindOnce(document.getElementById('agenda-type-filter'), 'change', () => renderAgendaWeek(ctx), '__agenda_type');
  bindOnce(document.getElementById('agenda-staff-filter'), 'change', () => renderAgendaWeek(ctx), '__agenda_staff');

  // bottone nuovo appuntamento (ID reale: agenda-new-appointment; mantengo anche agenda-new come fallback)
  const btnNew = document.getElementById('agenda-new-appointment') || document.getElementById('agenda-new');
  bindOnce(btnNew, 'click', () => {
